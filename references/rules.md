...
初始化调用 `.init` / `Type.init` 不作为方法声明跳过。显式类型数组中的简写初始化可沿用元素类型的商品语义；显式 productID 优先规则仍局限同一记录。只有能够关联到完整静态目录的商品引用可消除对应动态不确定性，交易结果读取不当作新的商品定义；其它未知购买来源、动态目录及拼接表达式仍需复核。
